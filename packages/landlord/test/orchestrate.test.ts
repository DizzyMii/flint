...
import { DependencyCycleError, orchestrate, resolveOrder } from '../src/orchestrate.ts';
...
describe('orchestrate', () => {
  it('single tenant completes end-to-end', async () => {
    ...
    const adapter = mockAdapter({
      onCall: (_req, i) => {
        if (i === 0)
          return toolCallResponse('emit_plan', {
            contracts: [
              {
                role: 'worker',
                objective: 'Do work',
    ...
    const adapter = mockAdapter({
      onCall: (_req, i) => {
        if (i === 0)
          return toolCallResponse('emit_plan', {
            contracts: [
              { role: 'w', objective: 'x', subPrompt: 'x', checkpoints: [], outputSchema: {} },
            ],
          });
        return textResponse('Done');
      },
