...
describe('openaiCompatAdapter -- shape', () => {
  it('name is "openai-compat"', () => {
    expect(openaiCompatAdapter({ baseUrl: 'https://api.openai.com/v1' }).name).toBe(
      'openai-compat',
    );
  });
...
describe('openaiCompatAdapter -- call() HTTP', () => {
  ...
  it('omits Authorization when no apiKey', async () => {
    ...
    await openaiCompatAdapter({ baseUrl: 'http://localhost:11434/v1', fetch }).call({
      model: 'llama3.2',
      messages: [{ role: 'user', content: 'Hi' }],
    });
...
describe('openaiCompatAdapter -- call() message normalization', () => {
  ...
  it('serializes assistant toolCalls as tool_calls with JSON-stringified arguments', async () => {
    ...
    const messages = getBody().messages as Array<Record<string, unknown>>;
    const toolCalls = (messages[1] as Record<string, unknown>).tool_calls as Array<
    ...
    expect(
      JSON.parse((toolCalls[0]?.function as Record<string, unknown>).arguments as string),
    ).toEqual({ x: 1 });
  });
  ...
  it('serializes multipart user message: text + image URL', async () => {
    ...
    const parts = (getBody().messages as Array<Record<string, unknown>>)[0]?.content as Array<
      Record<string, unknown>
    >;
  ...
  it('serializes base64 image as data URI in image_url', async () => {
    ...
    await openaiCompatAdapter({ baseUrl: 'https://api.openai.com/v1', fetch }).call({
      model: 'gpt-4o',
      messages: [
        { role: 'user', content: [{ type: 'image_b64', data: 'abc123', mediaType: 'image/png' }] },
      ],
    });
    const parts = (getBody().messages as Array<Record<string, unknown>>)[0]?.content as Array<
      Record<string, unknown>
  ...
  it('passes tool.jsonSchema as parameters in function definition', async () => {
    ...
    const fn = (getBody().tools as Array<Record<string, unknown>>)[0]?.function as Record<
      string,
      unknown
    >;
...
describe('openaiCompatAdapter -- call() response', () => {
  it('maps "stop" -> "end"', async () => {
    const res = await openaiCompatAdapter({
      baseUrl: 'https://api.openai.com/v1',
      fetch: mockFetch({ status: 200, body: mockResponse({ finish_reason: 'stop' }) }),
    }).call({ model: 'gpt-4o', messages: [{ role: 'user', content: 'Hi' }] });
  ...
  it('maps "length" -> "max_tokens"', async () => {
    const res = await openaiCompatAdapter({
      baseUrl: 'https://api.openai.com/v1',
      fetch: mockFetch({ status: 200, body: mockResponse({ finish_reason: 'length' }) }),
    }).call({ model: 'gpt-4o', messages: [{ role: 'user', content: 'Hi' }] });
  ...
  it('normalizes tool_calls into ToolCall[] with parsed arguments', async () => {
    ...
    expect(res.message.toolCalls?.[0]).toMatchObject({
      id: 'call_1',
      name: 'add',
      arguments: { a: 1, b: 2 },
    });
...
describe('openaiCompatAdapter -- stream()', () => {
  ...
  it('always emits usage and end chunks', async () => {
    ...
    const chunks: StreamChunk[] = [];
    for await (const c of openaiCompatAdapter({
      baseUrl: 'https://api.openai.com/v1',
      fetch: mockFetch(sse),
  ...
  it('accumulates token usage from the usage chunk in the stream', async () => {
    ...
    for await (const c of openaiCompatAdapter({
      ...
      chunks.push(c);
    const usageEvent = chunks.find((c) => c.type === 'usage') as
  ...
  it('maps "stop" in stream -> end reason "end"', async () => {
    ...
    for await (const c of openaiCompatAdapter({
      baseUrl: 'https://api.openai.com/v1',
      fetch: mockFetch(sseLines(streamChunk({}, 'stop'), '[DONE]')),
    }).stream({ model: 'gpt-4o', messages: [{ role: 'user', content: 'Hi' }] }))
      chunks.push(c);
    expect(
      (chunks.find((c) => c.type === 'end') as { type: 'end'; reason: string } | undefined)?.reason,
    ).toBe('end');
  });
  ...
  it('assembles streamed tool call argument deltas into a single tool_call chunk', async () => {
    ...
    for await (const c of openaiCompatAdapter({
      baseUrl: 'https://api.openai.com/v1',
      fetch: mockFetch(sse),
    }).stream({ model: 'gpt-4o', messages: [{ role: 'user', content: 'Weather?' }] }))
      chunks.push(c);
    const tc = chunks.filter((c) => c.type === 'tool_call')[0] as {
      type: 'tool_call';
  ...
  it('throws AdapterError on HTTP 401 before streaming begins', async () => {
    await expect(async () => {
      for await (const _ of openaiCompatAdapter({
        baseUrl: 'https://api.openai.com/v1',
        apiKey: 'bad',
        fetch: mockFetch({
          status: 401,
